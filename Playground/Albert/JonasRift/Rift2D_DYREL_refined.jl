...
function rectangular_perturbation!(T, xc, yc, r, xvi, thick_air, CharDim)
    ...
    ΔT = nondimensionalize(200.0e0K, CharDim)
    d_air = nondimensionalize(thick_air * km, CharDim)
...
function main2D(igg; ar = 8, ny = 16, nx = ny * 8, figdir = "figs2D", do_vtk = false)
    ...
    # Physical domain ------------------------------------
    thick_air = nondimensionalize(10.0e0km, CharDim)                 # thickness of sticky air layer
    ly = nondimensionalize(thickness, CharDim) + thick_air # domain length in y
    lx = nondimensionalize(1e3km, CharDim)           # domain length in x
    ...
    r_anomaly = nondimensionalize(200km, CharDim) # radius of perturbation
    ...
    # Rheology
    viscosity_cutoff = nondimensionalize((1.0e16Pa * s, 1.0e25Pa * s), CharDim)
    εbg = nondimensionalize(6.4e-16 / s, CharDim)
    stokes.ε.xx     .= εbg
    stokes.ε.xx_v   .= εbg
