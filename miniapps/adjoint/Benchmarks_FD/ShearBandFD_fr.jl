...
function main(igg; nx=64, ny=64, figdir="model_figs",f)
    ...
    dG = 1e-1
    pl_FD      = DruckerPrager_regularised(;  # non-regularized plasticity
    C    = C,
    ϕ    = ϕ + dG,
    η_vp = η_reg,
    Ψ    = 0
    ...
    rheology = (
        ...
        SetMaterialParams(;
            ...
            Density           = ConstantDensity(; ρ = 0.0),
            Gravity           = ConstantGravity(; g = 0.0),
            CompositeRheology = CompositeRheology((visc, el_bg, pl_FD)),
        ...
        SetMaterialParams(;
            ...
            Density           = ConstantDensity(; ρ = 0.0),
            Gravity           = ConstantGravity(; g = 0.0),
            CompositeRheology = CompositeRheology((visc, el_inc, pl_FD)),
        ),
    )
    ...
    #indx     = findall((xci[1] .>= 2*di[1]*f) .& (xci[1] .<= 1.0-(2*di[1]*f)))
    #indy     = findall((xvi[2] .>= 2*di[2]*f) .& (xvi[2] .<= 1.0-(2*di[2]*f))) 
