...
function main(li, origin, T_GMG, phases_GMG, igg; ε_bg = 1.0e-15, nx = 16, ny = 16, figdir = "figs2D", do_vtk = false)
    ...
    dt = nondimensionalize(10.0e3 * yr, CharDim) # initial timestep in seconds
    dtmax = nondimensionalize(10.0e3 * yr, CharDim) # initial timestep in seconds
    # ----------------------------------------------------
    ...
    initial_elevation = nondimensionalize(0.0e3 * m, CharDim)
    ...
    # pt_stokes        = PTStokesCoeffs(li_nd, di;  ϵ_abs=1e-3, ϵ_rel=1e-2, Re=3*√10*π, r=0.7, CFL = 0.98 / √2.1) # Re=3π, r=0.7
    pt_stokes = PTStokesCoeffs(li_nd, di; ϵ_abs = 1.0e-1, ϵ_rel = 1.0e-1, Re = 3 * √10 * π, r = 0.7, CFL = 0.98 / √2.1) # Re=3π, r=0.7
    ...
    Ttop = minimum(T_GMG_nd)
    Tbot = maximum(T_GMG_nd)
    thermal = ThermalArrays(backend_JR, ni)
    anomaly_center = nondimensionalize((0.0e0, -20.0e3) .* m, CharDim)
    anomaly_radius = nondimensionalize(5.0e3 * m, CharDim)
    ...
    pt_thermal = PTThermalCoeffs(
        backend_JR, rheology, phase_ratios, args0, dt, ni, di, li_nd; ϵ = 1.0e-5, CFL = 0.98 / √2.1
    )
    ...
    air_thickness = nondimensionalize(10.0e3 * m, CharDim)
    @views stokes.V.Vx[:, 2:(end - 1)] .= PTArray(backend_JR)([ εbg * x for x in xvi[1], y in xci[2]])
    @views stokes.V.Vy[2:(end - 1), :] .= PTArray(backend_JR)([-εbg * (y - air_thickness) for x in xci[1], y in xvi[2]])

    # vx_ext = nondimensionalize(8cm/yr, CharDim)
    # vy_ext = vx_ext / li_nd[1] * nondimensionalize(150km, CharDim)

    # @views stokes.V.Vx[:, 2:(end - 1)] .= PTArray(backend_JR)([ vx_ext / 2 * sign(x) for x in xvi[1], y in xci[2]])
    # @views stokes.V.Vy[2:(end - 1), :] .= PTArray(backend_JR)([ vy_ext for x in xci[1], y in xvi[2]])
    ...
    while t < tmax # run only for 5 Myrs
        ...
        pT.data[pPhases.data .== air_phase] .= Ttop
        clamp!(pT.data, Ttop, Tbot)
        ...
        println("           Δt:      $(ustrip(dt_dim / 1.0e3)) kyrs")
        ...
        if it == 1 || rem(it, 5) == 0
            ...
            supertitle = Label(fig2[0, :], "t = $(dimensionalize(t, yr, CharDim) / 1.0e3) kyr", fontsize = 30)
            # Plot temperature
...
figdir = "output2_fast_fixed_rheology_guet_$(n)/Rift2D_velocity"
nx, ny = n, div(n, 2)
...
ε_bg = 5.0e-15
main(li, origin, T_GMG, phases_GMG, igg; ε_bg = ε_bg, figdir = figdir, nx = nx, ny = ny, do_vtk = do_vtk);
