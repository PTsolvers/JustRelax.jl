# Using the APT method with auto tunned damping coefficients
...
Instead of using the Accelerated Pseudo-Transient where the damping coefficients are constant throughout the PT iterations (as in [Räss et al, 2022](https://gmd.copernicus.org/articles/15/5757/2022/)), we can use a self-tunning version of the APT method based on the approached described in [Duretz et al, 2025](https://egusphere.copernicus.org/preprints/2025/egusphere-2025-5641/).
...
1. The `PTStokesCoeffs` object containing the arrays needed for the standard APT solver is not needed anymore, and needs to be replaced by the `DYREL` object that contains all the new arrays that are needed for the self-tunned APT method. This means we need to change this line
```julia
...
- `nout` $\rightarrow$ damping coeffcients are re-computed every `nout` iterations.
- `rel_drop` $\rightarrow$ the tolerance for the inner dynamic relaxation loop is $error(P^n) \text{rel_drop}$ where $n$ is the outter Powell-Hesteness iteration counter.
- `λ_relaxation_PH` $\rightarrow$ relaxation coefficient for the plastic multiplier ($\cdot\lambda$) during the outter Powell-Hesteness loop. `λ_relaxation_PH=1` means no relaxation.
...
- `verbose_PH` $\rightarrow$ # print solver metrics during  outter Powell-Hesteness loop.
...
- `viscosity_cutoff` $\rightarrow$ viscosity is clamped so that $\text{viscosity_cutoff}_1 \leq \eta \leq \text{viscosity_cutoff}_1$.
