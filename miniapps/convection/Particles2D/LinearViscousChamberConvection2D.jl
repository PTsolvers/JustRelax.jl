...
include("LinearViscousChamber_rheologies.jl")
...
function main2D(igg; ar=8, ny=16, nx=ny*8, figdir="figs2D", save_vtk =false)
    ...
    # ly           = 10 + thick_air # domain length in y
    ly           = 10e3 # domain length in y
    lx           = 40e3           # domain length in x
    ...
    Cp = rheology[1].HeatCapacity[1].cp.val  # heat capacity     
    ρ0 = rheology[1].Density[1].ρ0.val        # reference Density
    k0 = rheology[1].Conductivity[1].k.val   # Conductivity
    G = rheology[1].Elasticity[1].G.val    # Shear Modulus
    ...
    yc_anomaly       = -0.75*ly  # origin of thermal anomaly
    r_anomaly        = 0.5e3    # radius of perturbation
    ...
    @parallel (@idx ni) compute_viscosity!(
        η, 1.0, phase_ratios.center, @strain(stokes)..., args, rheology, (1e14, 1e21)
    )
    η_vep            = copy(η)

    # PT coefficients for thermal diffusion
    pt_thermal       = PTThermalCoeffs(
        rheology, phase_ratios, args, dt, ni, di, li; ϵ=1e-5, CFL= 0.1 / √2.1
    )
    ...
    # Time loop
    t, it = 0.0, 0
    while it < 950 # run only for 5 Myrs
        ...
        if any(isnan, thermal.T)
            error("Check 2.1: NaNs in therml.T")
        end

        if any(isnan, thermal.Told)
            error("Check 2.2: NaNs in therml.Told")
        end

        grid2particle_flip!(pT, xvi, T_buffer, Told_buffer, particles.coords)
        if any(isnan, pT.data[particles.index.data[:]])
            error("Check 2.3: NaNs in pT")
        end
        # check if we need to inject particles
        inject = check_injection(particles)
        inject && inject_particles_phase!(particles, pPhases, (pT, ), (T_buffer,), xvi)
        if any(isnan, pT.data[particles.index.data[:]])
            error("Check 3: NaNs in pT after injection")
        end
        # update phase ratios
        @parallel (@idx ni) phase_ratios_center(phase_ratios.center, pPhases)
        ...
        save( joinpath(figdir, "check_point.png"), fig)
...
figdir   = "PIC_chamber_convection_1e14_dt0.3"
