...
function init_phases!(phases, particles, xc, yc, a, b, r, xc_anomaly, yc_anomaly, r_anomaly, sticky_air)
    ni = size(phases)

    @parallel_indices (i, j) function init_phases!(
        phases, px, py, index, xc, yc, a, b, r, xc_anomaly, yc_anomaly, r_anomaly, sticky_air)
    ...
    @parallel (JustRelax.@idx ni) init_phases!(
        phases, particles.coords..., particles.index, xc, yc, a, b, r, xc_anomaly, yc_anomaly, r_anomaly, sticky_air)
end
...
function elliptical_anomaly_gradient!(T, offset, xc, yc, a, b, r, xvi, sticky_air)

    @parallel_indices (i, j) function _elliptical_anomaly_gradient!(
        T, offset, xc, yc, a, b, r, x, y, sticky_air
    )
        depth = -y[j] - sticky_air
        @inbounds if (((x[i] - xc)^2 / ((a)^2)) + ((depth[j] + yc)^2 / ((b)^2)) ≤ r^2)
            T[i+1, j] += offset
    ...
    @parallel (1:nx-2, 1:ny) _elliptical_anomaly_gradient!(T, offset, xc, yc, a, b, r, xvi..., sticky_air)
end


function conduit_gradient!(T, offset, xc_conduit, yc_conduit, r_conduit, xvi, sticky_air)

    @parallel_indices (i, j) function _conduit_gradient!(
        T, offset, xc_conduit, yc_conduit, r_conduit, x, y, sticky_air
    )
        depth = -y[j] - sticky_air
        @inbounds if ((x[i] - xc_conduit)^2 ≤ r_conduit^2) && (0.325*(depth[j] + yc_conduit)^2 ≤ r_conduit^2)
            T[i+1, j] += offset
        end
        return nothing
    end
    ...
    @parallel (1:nx-2, 1:ny) _conduit_gradient!(T, offset, xc_conduit, yc_conduit, r_conduit, xvi..., sticky_air)
end

function conduit_gradient_TBuffer!(T, offset, xc_conduit, yc_conduit, r_conduit, xvi, sticky_air)
    ...
    @parallel (1:nx, 1:ny) _conduit_gradient_TBuffer!(T, offset, xc_conduit, yc_conduit, r_conduit, xvi..., sticky_air)
end
...
function open_conduit!(phases, particles, xc_conduit, yc_conduit, r_conduit, sticky_air)
    ni = size(phases)
    @parallel_indices (I...) function _open_conduit!(phases, px, py, index, xc_conduit, yc_conduit, r_conduit, sticky_air)
        ...
        @inbounds for ip in JustRelax.cellaxes(phases)
            ...
            x = JustRelax.@cell px[ip,I...]
            y = -(JustRelax.@cell py[ip,I...])- sticky_air
    ...
    @parallel (JustRelax.@idx ni) _open_conduit!(phases, particles.coords..., particles.index, xc_conduit, yc_conduit, r_conduit, sticky_air)
end
...
function new_thermal_anomaly(phases, particles, xc_anomaly, yc_anomaly, r_anomaly, sticky_air)
    ni = size(phases)

    @parallel_indices (I...) function new_anomlay_particles(phases, px, py, index, xc_anomaly, yc_anomaly, r_anomaly, sticky_air)
        @inbounds for ip in JustRelax.cellaxes(phases)
...
function main2D(igg; figdir=figdir, nx=nx, ny=ny)
    ...
    sticky_air  = 5e3              # thickness oif the sticky air layer
    ly          = 20e3 + sticky_air # domain length in y-direction
    lx          = 40e3            # domain length in x-direction
    ...
    x_anomaly = lx * 0.5
    y_anomaly = -ly * 0.35  # origin of the small thermal anomaly

    radius = 5e2         # radius of perturbation
    a = 15
    b = 5
    r_anomaly = 7.5e2             # radius of perturbation
    # r_anomaly = nondimensionalize(1.5km,CharDim)             # radius of perturbation
    xc_conduit, yc_conduit, r_conduit = (lx*0.5),4.15e3,2.3e3

    init_phases!(pPhases, particles, xc, yc, a, b, radius, x_anomaly, y_anomaly, r_anomaly,sticky_air)
    ...
    anomaly = 1000.0 .+ 273.0 # temperature anomaly
    offset  = 350.0.+273.0
    δT      = 20.0              # thermal perturbation (in %)
    elliptical_anomaly_gradient!(
            thermal.T, offset, xc, yc, a, b, radius, xvi, sticky_air
            )
    # conduit_gradient!(thermal.T, offset, xc_conduit, yc_conduit, r_conduit, xvi, sticky_air)
...
figdir = "New_benchmark_extension"
