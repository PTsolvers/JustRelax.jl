...
using JustRelax, JustRelax.JustRelax2D, JLD2
...
function main(igg; nx = 64, ny = 64, figdir = "model_figs")
    ...
    for _ in 1:20
        ...
        # Stokes solver ----------------
        stokes_walltime = @elapsed iters = solve_DYREL!(
            stokes,
            ρg,
            dyrel,
            flow_bcs,
        ...
        push!(pt_iterations, Int(maximum(iters.err_evo_it; init = 0)))
    ...
    jldsave(
        joinpath(figdir, "SB2D_CPU_$(nthreads)threads_$(nx)x$(ny).jld2");
        pt_iterations,
        stokes_walltimes,
        nthreads,
        grid_resolution = ni,
