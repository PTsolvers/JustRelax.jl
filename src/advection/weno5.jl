...
## Weno5 advection scheme. Implemantation based on the implementation of
...
## Upwind alphas
@inline weno_alphas_upwind(::WENO5, ::Type{Any}, β0, β1, β2) = error("Unknown method for the WENO Scheme")
...
## Downwind alphas
@inline weno_alphas_downwind(::WENO5, ::Any, β0, β1, β2)= error("Unknown method for the WENO Scheme")
...
# UP/DOWN-WIND FLUXES
@inline WENO_u_downwind(u1, u2, u3, u4, u5, weno) = _WENO_u(u1, u2, u3, u4, u5, weno, weno_alphas_downwind, stencil_candidate_downwind)
@inline WENO_u_upwind(u1, u2, u3, u4, u5, weno) = _WENO_u(u1, u2, u3, u4, u5, weno, weno_alphas_upwind, stencil_candidate_upwind)

@inline function _WENO_u(u1, u2, u3, u4, u5, weno, fun_alphas::F1, fun_stencil::F2) where {F1, F2}
...
## x-axis
@inline WENO_flux_downwind_x(u, nx, weno, i, j) = _WENO_flux_x(u, nx, weno, i, j, WENO_u_downwind)
@inline WENO_flux_upwind_x(u, nx, weno, i, j)   = _WENO_flux_x(u, nx, weno, i, j, WENO_u_upwind)
...
@inline function _WENO_flux_x(u, nx, weno, i, j, fun::F) where F
    ...
    u4 = @inbounds u[i, je]
    u5 = @inbounds u[i, jee]
    f = fun(u1, u2, u3, u4, u5, weno)
end

## y-axis
@inline WENO_flux_downwind_y(u, ny, weno, i, j) = _WENO_flux_y(u, ny, weno, i, j, WENO_u_downwind)
@inline WENO_flux_upwind_y(u, ny, weno, i, j) = _WENO_flux_y(u, ny, weno, i, j, WENO_u_upwind)
...
@parallel_indices (i, j) function weno_step3!(u, weno, Vxi, _di, ni, dt, one_third, two_thirds)
    rᵢ = weno_rhs(Vxi..., weno, _di..., ni..., i, j)
    @inbounds u[i, j] = @muladd one_third * u[i, j] + two_thirds * weno.ut[i, j] - two_thirds * dt * rᵢ
    return nothing
end
