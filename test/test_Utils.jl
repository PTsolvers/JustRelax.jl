...
@testset "getindex_NamedTuple" begin
    ...
    x_face = PTArray(backend_JR)([10.0i + j for i in 1:5, j in 1:4])   # (nx+1, ny)
    y_face = PTArray(backend_JR)([10.0i + j for i in 1:4, j in 1:5])   # (nx, ny+1)

    got = getidx((; x_face, y_face), 1, 1)
    @test got.x_face == Base.Array(x_face)[2, 1]
    @test got.y_face == Base.Array(y_face)[1, 2]

    # an explicit grid size wins over anything inferred from args
    ghosted = PTArray(backend_JR)([10.0i + j for i in 1:6, j in 1:6])  # (nx+2, ny+2)
    @test getidx((; ghosted), (4, 4), 1, 1).ghosted == Base.Array(ghosted)[2, 2]
end
