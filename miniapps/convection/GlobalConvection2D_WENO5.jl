...
function random_perturbation!(T, δT, xbox, ybox, xvi)
    ...
    ni = size(T) .- 2
    return @parallel (@idx ni) _random_perturbation!(T, δT, xbox, ybox, xvi...)
end
...
function thermal_convection2D(igg; ar = 8, ny = 16, nx = ny * 8, figdir = "figs2D", thermal_perturbation = :circular)
    ...
    while (t / (1.0e6 * 3600 * 24 * 365.25)) < 4.5e3
        ...
        if it == 1 || rem(it, 10) == 0
            ...
            h2 = heatmap!(ax2, xci[1] .* 1.0e-3, xvi[2] .* 1.0e-3, Array(stokes.V.Vy[2:(end - 1), 2:(end - 1)]), colormap = :batlow)
