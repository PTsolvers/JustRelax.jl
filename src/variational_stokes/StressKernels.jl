...
# 2D kernel
@parallel_indices (I...) function update_stresses_center_vertexDot!(
        ε::NTuple{3, T},      # normal components @ centers; shear components @ vertices
        ε_pl::NTuple{3},      # whole Voigt tensor @ centers
        EII,                  # accumulated plastic strain rate @ centers
        τ::NTuple{3},         # whole Voigt tensor @ centers
    ...
    if isvalid_v(ϕ, I...)
        ...
        frv = asind(sinϕv)
        frv = frv + ((dMv_ij*dp*frdot)* dt)
        sinϕv = sind(frv)
        cosϕv = cosd(frv)
        _Gvdt = inv((fn_ratio(get_shear_modulus, rheology, phase)+(dMv_ij*dp*Gdot))* dt)
        Kv = fn_ratio(get_bulk_modulus, rheology, phase) + (dMv_ij*dp*Kdot)
        volumev = isinf(Kv) ? 0.0 : Kv * dt * sinϕv * sinψv # plastic volumetric change K * dt * sinϕ * sinψ
        ηv_ij = av_clamped(η, Ic...)
        dτ_rv = inv(θ_dτ + ηv_ij * _Gvdt + 1.0)
    ...
    if all(I .≤ ni)
        if isvalid_c(ϕ, I...)
            ...
            phase = @inbounds phase_center[I...]
            dMij  = @inbounds dM[I...]
            _Gdt = inv((fn_ratio(get_shear_modulus, rheology, phase)+(dMij*dp*Gdot)) * dt)
            is_pl, C, sinϕ, cosϕ, sinψ, η_reg = plastic_params_phase(
                rheology, EII[I...], phase
            )
            fr = asind(sinϕ)
            fr = fr + ((dMij*dp*frdot)* dt)
            sinϕ = sind(fr)
            cosϕ = cosd(fr)
            K = fn_ratio(get_bulk_modulus, rheology, phase) + (dMv_ij*dp*Kdot)
            volume = isinf(K) ? 0.0 : K * dt * sinϕ * sinψ # plastic volumetric change K * dt * sinϕ * sinψ
            ηij = η[I...]
            dτ_r = 1.0 / (θ_dτ + ηij * _Gdt + 1.0)
