...
function init_rheologies2(CharDim; is_plastic = true)
    ...
    el_upper_mantle = SetConstantElasticity(; G = 67.0e9Pa, ν = 0.4)
    el_lower_mantle = SetConstantElasticity(; G = 67.0e9Pa, ν = 0.4)
    β_upper_crust   = inv(get_Kb(el_upper_crust))
    β_lower_crust   = inv(get_Kb(el_lower_crust))
    ...
    return rheology = (
        ...
        SetMaterialParams(;
            Phase = 5,
            Density = PT_Density(; ρ0 = (3.3e3 - 50)kg / m^3, β = β_upper_mantle, T0 = 0.0e0C, α = 5e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 1e3J / kg / K),
            Conductivity = K_mantle,
...
function init_phases!(phases, particles, Lx, d, r, thick_air, CharDim)
    ...
    d_35km = nondimensionalize(30.0e0km, CharDim)
    ...
    @parallel_indices (i, j) function init_phases!(phases, px, py, index, r, Lx)
        @inbounds for ip in cellaxes(phases)
            ...
            else #d_660km ≥ depth > d_120km
                @index phases[ip, i, j] = 4.0

            end

            # plume - rectangular
            if ((x - Lx * 0.5)^2 ≤ r^2) && ((depth - abs(d))^2 ≤ r^2)
