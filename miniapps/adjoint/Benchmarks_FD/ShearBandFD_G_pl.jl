...
function main(igg; nx=64, ny=64, figdir="model_figs",f)
    ...
    el_bg   = ConstantElasticity(; G=G0, ν=0.45)
    ...
    dG = 1e-2
    #el_bg_dG   = ConstantElasticity(; G=G0+dG, Kb=4.0)
    #el_inc_dG  = ConstantElasticity(; G=Gi+dG, Kb=4.0)
    el_bg_dG   = ConstantElasticity(; G=G0+dG, ν=0.45)
    el_inc_dG  = ConstantElasticity(; G=Gi+dG, ν=0.45)
    rheology = (
        # Low density phase
        SetMaterialParams(;
            Phase             = 1,
    ...
    for _ in 1:11
        ...
        save(joinpath(figdir, "$(t).png"), fig)
    end
    ...
    refcost = sum_kbn(BigFloat.(stokesP.V.Vy[indx,indy]))
    ...
    for (xit,i) in enumerate(xvi[1][1:end-1])
        ...
        if it == 1 || rem(it, 40) == 0
            ...
            h1 = heatmap!(ax1, xci..., Array(stokes.τ.II) , colormap=:batlow)
            # heatmap!(ax2, xci..., Array(log10.(stokes.viscosity.η_vep)) , colormap=:batlow)
            h2 = heatmap!(ax2, xci..., Array(log10.(stokes.EII_pl)) , colormap=:batlow)
...
nx     = n
ny     = n
figdir = "ShearBands2D_FD_elastic_G"
igg  = if !(JustRelax.MPI.Initialized())
    IGG(init_global_grid(nx, ny, 1; init_MPI = true)...)
else
    igg
end
refcost, cost, dp, Adjoint, ηref, ρref, stokesAD = main(igg; figdir = figdir, nx = nx, ny = ny,f);
...
function plot_FD_vs_AD(refcost,cost,dp,AD,nx,ny,ηref,ρref,stokesAD,figdir,f, Adjoint)
    ...
    r    =  2*di[1]*f  # half-width of block
    ...
    h3  = heatmap!(ax3, xci[1], xci[2], (Array(sol_FD) ./ Array(AD_G)))
    hidexdecorations!(ax1)
