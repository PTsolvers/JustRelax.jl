...
@parallel_indices (I...) function update_V_damping!(
        dVdτ::NTuple{N, AbstractArray{T, N}}, R::NTuple{N, AbstractArray{T, N}}, αV::NTuple{N, AbstractArray{T, N}}, ϕ::JustRelax.RockRatio
    ) where {N, T}
    _update_V_damping!(dVdτ, R, αV, ϕ, I...)
    return nothing
end

@generated function _update_V_damping!(
    ...
    quote
        ...
        Base.@ncall $N i -> begin
            ...
            if all(I .≤ size(R[i]))
                ...
                ϕᵢⱼ = ϕ_Vᵢ[i][inds...]
                if check
                    @inbounds dVdτ[i][I...] = ϕᵢⱼ * (αV[i][I...] * dVdτ[i][I...]) + R[i][I...]
                else
                    @inbounds dVdτ[i][I...] = 0.0
                end
            end
        end
    end
