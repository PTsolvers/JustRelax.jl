# 2D global mantle convection using GeoParams.jl

1. Define model length-scales 
```julia
Ra = 1e6
η_reg = 0.1
G0 = 1e6
MatParams = SetMaterialParams(;
        ...
        CompositeRheology=CompositeRheology(
                SetConstantElasticity(; G=G0, ν=0.5), # Elasticity
                ArrheniusType(), # T-dependant viscosity
                DruckerPrager_regularised(; C = 1e4, ϕ=0.0, η_vp=η_reg, Ψ=0.0) # non-regularized plasticity
            ),
...
3. Define non-dimensional physical domain $\Omega$
```julia
CharUnits = GEO_units(; length=2900km)
ly       = 2900km
lx       = ly * ar
