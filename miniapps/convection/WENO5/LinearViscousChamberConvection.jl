...
function main2D(igg; ar=8, ny=16, nx=ny*8, figdir="figs2D", save_vtk =false)
    ...
    while (t/(1e6 * 3600 * 24 *365.25)) < 5 # run only for 5 Myrs
        ...
        # advection_RK!(particles, @velocity(stokes), grid_vx, grid_vy, dt, 1)
        ...
        if it == 1 || rem(it, 10) == 0
            # # Make Makie figure
            # fig = Figure(resolution = (900, 900), title = "t=$(t/(1e6 * 3600 * 24 *365.25)) Myrs")
            # ax1 = Axis(fig[1, 1], title = "t=$(t/(1e6 * 3600 * 24 *365.25)) Myrs")
