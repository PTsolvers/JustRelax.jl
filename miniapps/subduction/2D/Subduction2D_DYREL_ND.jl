...
function main(li, origin, T_GMG, phases_GMG, igg; nx = 16, ny = 16, figdir = "figs2D", do_vtk = false)
    ...
    # while it < 100 # run only for 5 Myrs
        ...
        # compute time step
        dt = compute_dt(stokes, di, dt_max) * 0.9
        dt_dim = @dimstrip(dt, yr, CharDim) / 1e3
        println("Computed time step: $dt_dim kyr")
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            p        = particles.coords
            ppx, ppy = p
            pxv      = @dimstrip ppx.data[:] km CharDim
            pyv      = @dimstrip ppy.data[:] km CharDim
            clr      = @dimstrip pPhases.data[:] km CharDim
            idxv     = particles.index.data[:]

            xvi_dim = ntuple(i -> @dimstrip(xvi[i], km, CharDim), Val(2))
            xci_dim = ntuple(i -> @dimstrip(xci[i], km, CharDim), Val(2))
            ...
            # Plot temperature
            h1 = heatmap!(ax1, xvi_dim..., @dimstrip(Array(thermal.T[2:(end - 1), :]), C, CharDim), colormap = :batlow)
            ...
            h3 = heatmap!(ax3, xci_dim..., @dimstrip(Array(stokes.τ.II), Pa, CharDim), colormap = :batlow)
            # Plot effective viscosity
            h4 = heatmap!(ax4, xci_dim..., Array(log10.(@dimstrip(stokes.viscosity.η_vep, Pa*s, CharDim))), colormap = :batlow)
            # h4 = heatmap!(ax4, xci_dim..., Array(log10.(@dimstrip(stokes.viscosity.η, Pa*s, CharDim))), colormap = :batlow)
