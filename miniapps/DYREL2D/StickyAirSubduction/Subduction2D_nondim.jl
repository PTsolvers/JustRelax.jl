...
function main(li_dim, origin_dim, phases_GMG, igg; nx = 16, ny = 16, figdir = "figs2D", do_vtk = false)

    thickness = li_dim[2] * m
    η0 = 1.0e20
    # scales  = (τ=1e7, L=1e5, t=1e13) # kg = scale->S * scale->L * pow(scale->t,2.0);
    # julia> scales
    # (τ = 1.0e7, L = 100000.0, t = 1.0e13, ρ = 1.0e23, η = 1.0e20, V = 1.0e-8, a = 1.0e-21)
    CharDim = GEO_units(;
        # length = 1e5m, viscosity = η0
        length = 100000.0m, viscosity = η0 * Pa * s
    )

    # Physical domain ------------------------------------
    ni = nx, ny           # number of cells
    li = nondimensionalize(li_dim .* m, CharDim) # physical size of the domain
    origin = nondimensionalize(origin_dim .* m, CharDim) # physical size of the domain
    di = @. li / ni       # grid steps
    ...
    dt = nondimensionalize(1.0e3yr, CharDim) # diffusive CFL timestep limiter
    # ----------------------------------------------------
    ...
    while it < 15 # run only for 5 Myrs
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            h3 = heatmap!(ax3, xci[1] .* 1.0e-3, xci[2] .* 1.0e-3, (@dimstrip stokes.V.Vy cm / yr CharDim), colormap = :batlow)
            # Plot effective viscosity
            h4 = heatmap!(ax4, xci[1] .* 1.0e-3, xci[2] .* 1.0e-3, Array(ρg[2]), colormap = :batlow)
...
n = 128
nx, ny = (250, 100) .÷ 2
li_dim, origin_dim, phases_GMG, T_GMG = GMG_subduction_2D(nx + 1, ny + 1)
...
main(li_dim, origin_dim, phases_GMG, igg; figdir = figdir, nx = nx, ny = ny, do_vtk = do_vtk);
