...
# harmonic averages
@inline    _harm(A::T, i, j) where {T<:T2} = 4.0 * sum(inv(A[ii, jj]) for ii in i+1:i+2, jj in j+1:j+2)
@inline _harm_xa(A::T, i, j) where {T<:T2} = 2.0 * (inv(A[i + 1, j    ]) + inv(A[i, j]))
@inline _harm_ya(A::T, i, j) where {T<:T2} = 2.0 * (inv(A[i    , j + 1]) + inv(A[i, j]))
#others
...
# harmonic averages
@inline  _harm_x(A::T, i, j, k) where {T<:T3} = 2.0 * inv(inv(A[i + 1, j, k]) + inv(A[i, j, k]))
@inline  _harm_y(A::T, i, j, k) where {T<:T3} = 2.0 * inv(inv(A[i, j + 1, k]) + inv(A[i, j, k]))
@inline  _harm_z(A::T, i, j, k) where {T<:T3} = 2.0 * inv(inv(A[i, j, k + 1]) + inv(A[i, j, k]))
@inline _harm_xy(A::T, i, j, k) where {T<:T3} = 4.0 * inv(sum(inv(A[ii, jj, k]) for ii in i:i-1, jj in j:j-1))
@inline _harm_xz(A::T, i, j, k) where {T<:T3} = 4.0 * inv(sum(inv(A[ii, j, kk]) for ii in i:i-1, kk in k:k-1))
@inline _harm_yz(A::T, i, j, k) where {T<:T3} = 4.0 * inv(sum(inv(A[i, jj, kk]) for jj in j:j-1, kk in k:k-1))
# others
