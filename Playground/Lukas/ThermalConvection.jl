...
function main2D(igg; ar = 1, nx = 32, ny = 32, nit = 1.0e1, figdir = "figs2D", do_vtk = false, finalize_MPI = true)
    ...
    ly = nondimensionalize(thickness, CharDim)   # domain length in y
    ...
    @show dt
    dt = dt_diff = 1 / ( 2 * (1/di[1]^2 + 1/di[2]^2))
    @show dt
    # ----------------------------------------------------
    ...
    yc_anomaly = 2 / 5  # origin of thermal anomaly
    r_anomaly = 0.1     # radius of perturbation
    ...
    dyrel = DYREL(backend_JR, stokes, rheology, phase_ratios, grid.di, dt; ϵ = 1e-9)
