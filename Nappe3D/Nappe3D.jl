...
function inrectangle(x, y, z, width, length, height, center)
    (∈)(x, lims::NTuple{2}) = lims[1] ≤ x ≤ lims[2]

    xlims = (-width/2, width/2)   .+ center[1]
    ylims = (-length/2, length/2) .+ center[2]
    zlims = (-height/2, height/2) .+ center[3]
...
function init_phases!(phases, particles, anomaly_args)
    ni = size(phases)

    @parallel_indices (I...) function init_phases!(
        phases, px, py, pz, index, anomaly_args
    )
        i, j, k = I
        @inbounds for ip in JustRelax.cellaxes(phases)
            ...
            @cell phases[ip, I...] = 1.0 # lithosphere

            # Nappe
            if inrectangle(x[i], y[j], z[k], anomaly_args...)
...
function thermal_perturbation!(T, δT, xc_anomaly, yc_anomaly, zc_anomaly, r_anomaly, xvi)

    @parallel_indices (i, j, k) function add_perturbation!(
        T, δT, x, y, z, anomaly_args
    )
        # depth = -z[k] - sticky_air
        
        @inbounds if inrectangle(x[i], y[j], z[k], anomaly_args...)
            T[i, j, k] = δT
        end
        return nothing
    end
...
function init_rheology(CharDim; is_compressible = false)
    ...
    # end
    creep_nappe        = LinearViscous(; η=1e20 * Pa * s)
    creep_lithosphere  = LinearViscous(; η=1e18 * Pa * s)
    ρ0_lithosphere     = 2800
    ρ0_nappe           = ρ0_lithosphere * 0.9
    ...
    rheology = (
        ...
        SetMaterialParams(;
            Phase             = 1,
            # Density           = PT_Density(; ρ0=ρ0_lithosphere * (kg / m^3), α=3e-5 / K, T0=0.0C),
            Density           = ConstantDensity(; ρ=ρ0_lithosphere * (kg / m^3)),
            HeatCapacity      = ConstantHeatCapacity(; Cp=1050J / kg / K),
            Conductivity      = ConstantConductivity(; k=3.0Watt / K / m),
            LatentHeat        = ConstantLatentHeat(; Q_L=350e3J / kg),
            ShearHeat         = ConstantShearheating(0.0NoUnits),
            CompositeRheology = CompositeRheology((creep_lithosphere, )),
        ...
        SetMaterialParams(;
            Phase             = 2,
            # Density           = PT_Density(; ρ0=ρ0_nappe * (kg / m^3), T0=0.0C),
            Density           = ConstantDensity(; ρ=ρ0_nappe * (kg / m^3)),
            HeatCapacity      = ConstantHeatCapacity(; Cp=1050J / kg / K),
            Conductivity      = ConstantConductivity(; k=1.5Watt / K / m),
            LatentHeat        = ConstantLatentHeat(; Q_L=350e3J / kg),
            ShearHeat         = ConstantShearheating(0.0NoUnits),
            CompositeRheology = CompositeRheology((creep_nappe, )),
            # Melting           = MeltingParam_Caricchi(),
            Gravity           = ConstantGravity(; g=g),
            # Elasticity        = el_magma,
            CharDim           = CharDim,
...
function main3D(igg; figdir = "output", nx = 64, ny = 64, nz = 64, do_vtk = false)
    ...
    CharDim      = GEO_units(;length=100km, viscosity=1e20, temperature = 1e3C)

    #-------JustRelax parameters-------------------------------------------------------------
    # Domain setup for JustRelax
    sticky_air   = nondimensionalize(0km, CharDim)   # thickness of the sticky air layer
    lz           = nondimensionalize(100km,CharDim)  # domain length in y-direction
    lx      = ly = nondimensionalize(100km, CharDim) # domain length in x-direction
    ...
    # Nappe geometry & location
    length_x   = 0.1 * lx / 2
    length_y   = 0.1 * ly
    length_z   = 0.4 * lz /2
    center     = (lx / 2, ly / 2, -lz + length_z/2 + lz*0.1)
    nappe_args = (length_x, length_y, length_z, center)
    ...
    init_phases!(pPhases, particles, nappe_args)
    ...
    @parallel (@idx ni .+ 1) init_T!(
        ...
        nondimensionalize((1200 - 273)K,CharDim) / nondimensionalize(100km,CharDim),
