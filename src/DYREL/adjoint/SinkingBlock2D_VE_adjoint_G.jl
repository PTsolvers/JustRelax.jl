...
function sinking_block2D_VE(
    ...
    differentiate_G = adjoint || !isnothing(G_multiplier)
    controls, gradients = material_controls(backend, ni, differentiate_G ? (:G,) : ())
    if !isnothing(G_multiplier)
        size(G_multiplier) == ni || throw(DimensionMismatch("G_multiplier must match the center grid"))
        copyto!(controls.G.center, G_multiplier)
    ...
    while it <= nt
        ...
        if plot_results && it in (0, nt)
            ...
            nrows = AdjointSolve ? 3 : 2
            f = Figure(size = (1900, AdjointSolve ? 1400 : 950))
            ...
            if AdjointSolve
                # The center field is the complete dJ/dG: it includes the vertex
                # contribution pulled back through center2vertex!.
                panel!(
                    3, 1, "∂J/∂G", xc, gradients.G.center;
    ...
    return return_fields ? (;
            cost,
            G_gradient = adjoint ? Array(gradients.G.center) : nothing,
            shear_modulus = G,
