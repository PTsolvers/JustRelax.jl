...
function DYREL(ni::NTuple{2}; CFL= 0.99, c_fat = 0.5)
    ...
    T = typeof(γ_eff)
    F = typeof(CFL)
    DYREL{T, F}(γ_eff, Dx, Dy, λmaxVx, λmaxVy, dVxdτ, dVydτ, dτVx, dτVy,
                     dVx, dVy, βVx, βVy, cVx, cVy, αVx, αVy, ηb, CFL, c_fat)
end

function DYREL(stokes::JustRelax.StokesArrays, rheology, phase_ratios, di, dt; CFL= 0.99, c_fat = 0.5)
    
    ni = size(stokes.P)
    
    # instantiate DYREL object
    dyrel = DYREL(ni; CFL=CFL, c_fat=c_fat)
    # compute bulk viscosity and penalty parameter
    compute_bulk_viscosity_and_penalty!(dyrel, stokes, rheology, phase_ratios, dt)
...
function compute_bulk_viscosity_and_penalty!(dyrel, stokes, rheology, phase_ratios, dt)
    @parallel compute_bulk_viscosity_and_penalty!(dyrel.ηb, dyrel.γ_eff, rheology, phase_ratios.center, mean(stokes.viscosity.η), dt)
    return nothing
end

@parallel_indices (I...) function compute_bulk_viscosity_and_penalty!(ηb, γ_eff, rheology, phase_ratios_center, η_mean, dt)
