...
@views function Stokes2D_VEP(n)
    ...
    η0       = 1e0/sc.σ/sc.t       # viscous viscosity
    ηi       = 1e0/sc.σ/sc.t       # min/max inclusion viscosity
    G0       = 1e0/sc.σ
    Gi       = 1/2/sc.σ/sc.t       # min/max inclusion viscosity
    C        = 1.6/cosd(30)/sc.σ
    Δt       = η0/G0/4
    ...
    ηvp      = 1e-2/sc.σ/sc.t    
    # Numerics
    ncx, ncy = 64, 64       # numerical grid resolution
    nt       = 1            # time steps
    ϵ        = 1e-6         # tolerance
    ...
    for it=1:nt
        ...
        p1 = Plots.heatmap(xc, yc, log10.(EIIc'./sc.t) , aspect_ratio=1, c=:inferno, title="EII", xlims=(-Lx/2,Lx/2), xlabel="x",ylabel="y")
        p2 = Plots.heatmap(xc, yc, Pt'.*sc.σ , aspect_ratio=1, c=:inferno, title="Pt", xlims=(-Lx/2,Lx/2), xlabel="x",ylabel="y")
        ...
        p4 = Plots.plot(LinRange(Δt, it*Δt, it), Tii_evo[1:it]*sc.σ, xlabel="time",ylabel="mean dev. stress", label=:none)
        # p4 = Plots.heatmap(xc, yc, log10.(ηc)' , aspect_ratio=1, c=:inferno, title="ηc", xlims=(-Lx/2,Lx/2))
