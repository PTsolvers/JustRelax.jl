...
function init_rheologies(CharDim; is_plastic = true)
    ...
    disl_upper_crust = DislocationCreep(
        A = 8.57e-18Pa^(-4) / s, # units are Pa^(-n) / s
        n = 4e0NoUnits,
        E = 223.0e3J / mol,
        V = 0.0e-6m^3 / mol,
        r = 0.0NoUnits,
    ...
    diff_lithospheric_mantle = DiffusionCreep(
        A = 2.46e-16Pa^(-1) / s, # units are Pa^(-n - r) / s * m^(-p)
        n = 1.0NoUnits,
        E = 375.0e3J / mol,
        V = 1.0e-5m^3 / mol,
        d = 1mm,
    ...
    el_lithospheric_mantle    = SetConstantElasticity(; G = 67.0e9Pa, ν = 0.25)
    el_sublithospheric_mantle = SetConstantElasticity(; G = 67.0e9Pa, ν = 0.25)
    ...
    return rheology = (
        ...
        SetMaterialParams(;
            Phase = 1,
            Density = PT_Density(; ρ0 = 2.65e3kg / m^3, β = β_upper_crust, T0 = 0.0e0C, α = 3.5e-5 / K),
            ...
            ShearHeat = ConstantShearheating(1),
        ...
        SetMaterialParams(;
            Phase = 2,
            Density = PT_Density(; ρ0 = 2.76e3kg / m^3, β = β_upper_crust, T0 = 0.0e0C, α = 3.5e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 7.5e2J / kg / K),
            Conductivity = K_crust,
        ...
        SetMaterialParams(;
            Phase = 3,
            Density = PT_Density(; ρ0 = 3.3e3kg / m^3, β = β_upper_crust, T0 = 0.0e0C, α = 5e-5 / K),
        ...
        SetMaterialParams(;
            Phase = 4,
            Density = PT_Density(; ρ0 = (3.3e3 - 50)kg / m^3, β = β_upper_crust, T0 = 0.0e0C, α = 5e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 1.25e3J / kg / K),
            Conductivity = K_mantle,
            RadioactiveHeat = ConstantRadioactiveHeat(0.0),
        ...
        SetMaterialParams(;
            Phase = 5,
             Density = PT_Density(; ρ0 = (3.3e3 - 30)kg / m^3, β = β_upper_crust, T0 = 0.0e0C, α = 5e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 1.25e3J / kg / K),
            Conductivity = K_mantle,
            RadioactiveHeat = ConstantRadioactiveHeat(0.0),
            CompositeRheology = CompositeRheology((disl_sublithospheric_mantle, diff_sublithospheric_mantle, el_sublithospheric_mantle)),
...
function init_rheologies2(CharDim; is_plastic = true)
    ...
    # Elasticity
    el_upper_crust  = SetConstantElasticity(; G = 25.0e9Pa,  ν = 0.36)
    el_lower_crust  = SetConstantElasticity(; G = 25.0e9Pa,  ν = 0.36)
    el_upper_mantle = SetConstantElasticity(; G = 80.0e9Pa,  ν = 0.36)
    el_lower_mantle = SetConstantElasticity(; G = 120.0e9Pa, ν = 0.36)
    ...
    return rheology = (
        ...
        SetMaterialParams(;
            ...
            HeatCapacity = ConstantHeatCapacity(; Cp = 7.4e2J / kg / K),
            Conductivity = ConstantConductivity(2.7Watt/m/K),
            CompositeRheology = CompositeRheology((disl_upper_crust, el_upper_crust, pl_crust)),
            Elasticity = el_upper_crust,
            RadioactiveHeat = ConstantRadioactiveHeat(1.1e-9Watt / kg),
            Gravity = ConstantGravity(; g = g),
            ShearHeat =  ConstantShearheating(1),
            CharDim = CharDim,
        ),
        ...
        SetMaterialParams(;
            ...
            HeatCapacity = ConstantHeatCapacity(; Cp = 7.11e2J / kg / K),
            Conductivity = ConstantConductivity(2.7Watt/m/K),
            RadioactiveHeat = ConstantRadioactiveHeat(1.1e-9Watt / kg),
            CompositeRheology = CompositeRheology((disl_lower_crust, el_lower_crust, pl_crust)),
            Gravity = ConstantGravity(; g = g),
        ...
        SetMaterialParams(;
            ...
            Conductivity = ConstantConductivity(3e0Watt/m/K),
            CompositeRheology = CompositeRheology((disl_lithospheric_mantle, diff_lithospheric_mantle, el_upper_mantle, pl)),
            Gravity = ConstantGravity(; g = g),
            RadioactiveHeat = ConstantRadioactiveHeat(0e0Watt / kg),
            ShearHeat =  ConstantShearheating(1),
        ...
        SetMaterialParams(;
            ...
            Density = PT_Density(; ρ0 = 3.3e3kg / m^3, β = β_upper_mantle, T0 = 0.0e0C, α = 3e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 1.2e3J / kg / K),
            Conductivity = ConstantConductivity(3.3e0Watt/m/K),
            CompositeRheology = CompositeRheology((disl_lithospheric_mantle, diff_lithospheric_mantle, el_upper_mantle, pl)),
            Gravity = ConstantGravity(; g = g),
            RadioactiveHeat = ConstantRadioactiveHeat(0e0Watt / kg),
            ShearHeat =  ConstantShearheating(1),
        ...
        SetMaterialParams(;
            Phase = 5,
            Density = PT_Density(; ρ0 = (3.3e3 - 25)kg / m^3, β = β_upper_mantle, T0 = 0.0e0C, α = 3e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = 1.2e3J / kg / K),
            Conductivity = ConstantConductivity(3.3e0Watt/m/K),
            CompositeRheology = CompositeRheology((disl_sublithospheric_mantle, diff_sublithospheric_mantle, el_upper_mantle, pl)),
            Gravity = ConstantGravity(; g = g),
            RadioactiveHeat = ConstantRadioactiveHeat(0e0Watt / kg),
            ShearHeat =  ConstantShearheating(1),
            Elasticity = el_upper_mantle,
        ...
        SetMaterialParams(;
            Phase = 6,
            Density = PT_Density(; ρ0 = 1kg / m^3, β = β_upper_mantle, T0 = 0.0e0C, α = 3e-5 / K),
            HeatCapacity = ConstantHeatCapacity(; Cp = (1.2e3 * 100)J / kg / K),
            Conductivity = ConstantConductivity(3.3e0Watt/m/K),
            CompositeRheology = CompositeRheology((LinearViscous(; η=1e19Pa*s), el_upper_crust)),
            Gravity = ConstantGravity(; g = g),
            RadioactiveHeat = ConstantRadioactiveHeat(0e0Watt / kg),
            ShearHeat =  ConstantShearheating(0),
            # Elasticity = el_upper_crust,
            CharDim = CharDim,
        ),
        # Name              = "WZ",
...
function init_phases!(phases, particles, Lx, d, r,  CharDim)
    ni = size(phases)

    d_0km = nondimensionalize(0.0e0km, CharDim)
    d_21km = nondimensionalize(20.0e0km, CharDim)
    ...
    d_WZ = nondimensionalize(5.0e0km, CharDim)
    @parallel_indices (i, j) function init_phases!(phases, px, py, index, r, Lx)
        @inbounds for ip in cellaxes(phases)
            ...
            if  depth < 0
                @index phases[ip, i, j] = 6.0 # sticky air crust
                
            elseif d_0km ≤ depth ≤ d_21km
                @index phases[ip, i, j] = 1.0 # upper crust

            elseif d_35km ≥ depth > d_21km
                @index phases[ip, i, j] = 2.0 # lower crust

            elseif d_120km ≥ depth > d_35km
                @index phases[ip, i, j] = 3.0 # lithospheric mantle

                # WZ - rectangular
                if (x - Lx * 0.5)^2 ≤ d_WZ^2
                    @index phases[ip, i, j] = 7.0
                end

            else
                @index phases[ip, i, j] = 4.0 # upper mantle

            end

            # plume - rectangular
            if ((x - Lx * 0.5)^2 + (depth - abs(d))^2 ≤ r^2) # mantle plmue
...
function phase_changes!(phases, pP, pT, particles, phase_upper_mantle, phase_lower_mantle, CharDim)
    ni = size(phases)

    d_660km = nondimensionalize(660.0e0km, CharDim)

    γ  = nondimensionalize(-2MPa/K, CharDim)
    P0 = nondimensionalize(23.5GPa, CharDim)
    T0 = nondimensionalize(1350C, CharDim)

    @parallel_indices (i, j) function _phase_changes!(phases, pP, PT, P0, T0, py, index, phase_upper_mantle, phase_lower_mantle)
        @inbounds for ip in cellaxes(phases)
            # quick escape
            @index(index[ip, i, j]) == 0 && continue

            depth = -(@index py[ip, i, j])
            P = @index pP[ip, i, j] # particles pressure
            T = @index pT[ip, i, j] # particles pressure
            Peq = P0 + γ * (T - T0)
            ...
            if P ≤ Peq && @index(phases[ip, i, j]) == 5.0
    ...
    return @parallel (@idx ni) _phase_changes!(phases, pP, pT, P0, T0, particles.coords[end], particles.index, phase_upper_mantle, phase_lower_mantle)
end
