...
@testset "DYREL" begin
    @testset "Adjoint Stokes allocator" begin
        ...
        @test size(adjoint3D.dρgx) == (3, 2, 4)
    ...
    @testset "DYREL 2D allocator" begin
        # NTuple constructor
        nx, ny = 8, 6
        dyrel = JustRelax2D.DYREL(backend, (nx, ny); ϵ = 1.0e-7, ϵ_vel = 2.0e-7, CFL = 0.5, c_fact = 0.25, γfact = 31.0)
        ...
        dyrel_forwarded = JustRelax2D.DYREL(backend, nx, ny; CFL = 0.7, γfact = 31.0)
    ...
    @testset "DYREL struct wrappers" begin
        ...
        nx, ny = 5, 4
        dyrel = JustRelax2D.DYREL(backend, (nx, ny); CFL = 0.8)
        dyrel.dτVx .= 0.5
        dyrel.dτVy .= 0.5
        dyrel.cVx .= 0.1
        dyrel.cVy .= 0.1
