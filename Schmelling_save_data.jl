...
function sortfiles(pth) 
    x = filter(x->!contains(x, "walltime"), pth)
    x[sortperm(get_niter.(x))]
end

ni = [(125, 50) .* i for i in  1:10]

pth = ["/Users/albert/Documents/Schmeling/tmp/Schmeling_APT_$(n[1])x$(n[2])/its/walltime.jld2" for n in ni]
wt_apt   = [jldopen(pth)["t_stokes"] for pth in pth]
Myrs_apt = [jldopen(pth)["t_Myrs"] for pth in pth]

pth = ["/Users/albert/Documents/Schmeling/tmp/Schmeling_DR_$(n[1])x$(n[2])/its/walltime.jld2" for n in ni]
wt_dr   = [jldopen(pth)["t_stokes"] for pth in pth]
Myrs_dr = [jldopen(pth)["t_Myrs"] for pth in pth]

pth     = [readdir("/Users/albert/Documents/Schmeling/tmp/Schmeling_APT_$(n[1])x$(n[2])/its/"; join=true) for n in ni]
fls_apt = sortfiles.(pth)
pth     = [readdir("/Users/albert/Documents/Schmeling/tmp/Schmeling_DR_$(n[1])x$(n[2])/its/"; join=true) for n in ni]
