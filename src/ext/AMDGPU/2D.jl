module JustRelax2D
    ...
    import JustRelax:
        IGG, BackendTrait, CPUBackendTrait, AMDGPUBackendTrait, backend, CPUBackend, Geometry, @cell
    ...
    function JR2D.StokesArrays(
        ::Type{AMDGPUBackend}, ni::NTuple{N,Integer}
    ) where {N}
        return StokesArrays(ni)
    end

    function JR2D.ThermalArrays(
        ::Type{AMDGPUBackend}, ni::NTuple{N,Number}
    ) where {N}
        return ThermalArrays(ni...)
    end

    function JR2D.ThermalArrays(
        ::Type{AMDGPUBackend}, ni::Vararg{Number,N}
    ) where {N}
        return ThermalArrays(ni...)
    end
    ...
    function JR2D.phase_ratios_center(
        ::AMDGPUBackendTrait, phase_ratios::JustRelax.PhaseRatio, particles, grid::Geometry, phases
    )
    ...
    function JR2D.compute_viscosity!(
        ::AMDGPUBackendTrait, stokes, ν, args, rheology, cutoff
    )
    ...
    function JR2D.temperature2center!(
        ::AMDGPUBackendTrait, thermal::JustRelax.ThermalArrays
    )
