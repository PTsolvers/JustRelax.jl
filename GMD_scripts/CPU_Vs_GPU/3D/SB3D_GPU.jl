...
using JustRelax, JustRelax.JustRelax3D, JustRelax.DataIO, JLD2
...
function main(igg; nx = 64, ny = 64, nz = 64, figdir = "model_figs")
    ...
    pt_stokes = PTStokesCoeffs(li, di; ϵ_abs = 1.0e-5, ϵ_rel = 1.0e-5, CFL = 0.5 / √3.1)
    # Buoyancy forces
    ...
    ttot = Float64[]
    pt_iterations = Int[]
    stokes_walltimes = Float64[]

    while t < tmax
        # Stokes solver ----------------
        stokes_walltime = @elapsed iters = solve!(
            stokes,
            pt_stokes,
            grid,
            flow_bcs,
        ...
        push!(pt_iterations, iters.iter)
        push!(stokes_walltimes, stokes_walltime)
    ...
    jldsave(
        joinpath(figdir, "SB3D_GPU_$(nthreads)threads_$(nx)x$(ny)x$(nz).jld2");
        pt_iterations,
        stokes_walltimes,
        nthreads,
        grid_resolution = ni,
