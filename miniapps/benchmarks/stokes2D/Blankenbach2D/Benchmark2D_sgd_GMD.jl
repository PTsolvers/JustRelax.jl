...
using Printf, LinearAlgebra, GeoParams, CairoMakie, CellArrays, MPI
...
function main2D(igg; ar = 1, nx = 32, ny = 32, nit = 1.0e1, figdir = "figs2D", do_vtk = false, finalize_MPI = true)
    ...
    # @show Urms[Int64(it)] Nu_top[Int64(it)]
...
elseif runtype == :multiple
    ...
    for i in nrange
        ...
        Urms, Nu_top, trms, T, xvi = main2D(igg; figdir = figdir, ar = 1, nx = nx, ny = ny, nit = 10.0e3, do_vtk = false, finalize_MPI = false)

        # Store results for each resolution
        all_results[n] = (Urms = Urms, Nu_top = Nu_top, trms = trms, T = T, xvi = xvi)
    ...
    fig3 = Figure(size = (800, 800), fontsize = 20)
    ax31 = Axis(
        fig3[1:2, 1:2], aspect = DataAspect(),
        # title = "t=$(round(t / (1.0e6 * 3600 * 24 * 365.25); digits = 2)) Myrs",
        title = L"Blankenback Benchmark - Temperature [K]",
        titlesize = 40,
        yticklabelsize = 20,
        xticklabelsize = 20,
        xlabelsize = 20,
    ...
    contour!(ax31, xvi[1] .* 1.0e-3, xvi[2] .* 1.0e-3, Array(T[2:(end - 1), :]), levels = 4, color = :white, linewidth = 1)
    # Colorbar(fig3[1:2, 3], hm; height = Relative(0.75), label = "T [K]", labelsize = 14, ticklabelsize = 12)
