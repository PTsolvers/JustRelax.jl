...
function init_rheologies(; is_plastic = true)
    ...
    diff_lithospheric_mantle = DiffusionCreep(A = 2.51e-17, n = 1.0, E = 530.0e3, V = 6.0e-6, p = 0, r = 0.0, R = 8.3145)
    diff_sublithospheric_mantle = DiffusionCreep(A = 2.51e-17, n = 1.0, E = 530.0e3, V = 6.0e-6, p = 0, r = 0.0, R = 8.3145)
    ...
    return rheology = (
        ...
        SetMaterialParams(;
            Phase = 5,
            Density = ConstantDensity(; ρ = 1.0e3), # water density
...
function init_phases!(phases, particles, Lx, d, r, thick_air)
    ...
    @parallel_indices (I...) function init_phases!(phases, px, py, index, r, Lx)
        @inbounds for ip in cellaxes(phases)
            # quick escape
            @index(index[ip, I .+ 1...]) == 0 && continue

            x = @index px[ip, I .+ 1...]
            depth = -(@index py[ip, I .+ 1...]) - thick_air
            ...
            # plume - rectangular
            if ((x - Lx * 0.5)^2 ≤ r^2) && (((@index py[ip, I .+ 1...]) - d - thick_air)^2 ≤ r^2)
