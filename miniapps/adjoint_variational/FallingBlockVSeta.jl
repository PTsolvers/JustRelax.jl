...
function main(igg; nx=64, ny=64, figdir="model_figs",f,run_param)
    ...
    # Stokes solver ----------------
    Dot = adjoint_solve_VariationalStokes!(     # Change adjoint_solveDot! again!!!!
        stokesDot,
        stokesAD,
        pt_stokes,
        di,
    ...
    return refcost, cost, dp, dp_m, Adjoint, ηref, ρref, stokesAD, stokesRef, refcostdot, ρg
end
...
ny     = 16*f
run_param = true
figdir = "miniapps/adjoint_variational/FallingBlockVSeta/"
igg  = if !(JustRelax.MPI.Initialized())
    IGG(init_global_grid(nx, ny, 1; init_MPI = true)...)
else
    igg
end
refcost, cost, dp, dp_m, Adjoint, ηref, ρref, stokesAD, stokesRef, refcostdot, ρg = main(igg; figdir = figdir, nx = nx, ny = ny,f,run_param);

#which sensitivity to plot
plot_sens = stokesAD.η .* refcost
