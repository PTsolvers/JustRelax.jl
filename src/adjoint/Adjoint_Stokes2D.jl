...
# backend trait
function adjoint_solveDot!(stokes::JustRelax.StokesArrays, stokesAD::JustRelax.StokesArraysAdjoint, args...; kwargs)
    return adjoint_solveDot!(backend(stokes), stokes, stokesAD, args...; kwargs)
end

# entry point for extensions
adjoint_solveDot!(::CPUBackendTrait, stokes, stokesAD, args...; kwargs) = _adjoint_solveDot!(stokes, stokesAD, args...; kwargs...)

function _adjoint_solveDot!(
    stokes::JustRelax.StokesArrays,
    stokesAD::JustRelax.StokesArraysAdjoint,
    pt_stokes,
    di::NTuple{2,T},
    ...
    while iter ≤ iterMax
        ...
        wtime0 += @elapsed begin
            ...
            #stokes.viscosity.η .+= (dp_m*dp)
            #print(extrema(stokes.viscosity.η),"\n")
