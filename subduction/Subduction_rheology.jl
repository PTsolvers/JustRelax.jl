...
function init_rheologies()
    ...
    rheology = (
        ...
        SetMaterialParams(;
            Phase             = 2,
            Density           = PT_Density(; ρ0=2.75e3, α = α, T0 = 273),
            CompositeRheology = CompositeRheology( 
                (
                    disl_wet_quartzite, 
                    diff_wet_quartzite,
        ...
        SetMaterialParams(;
            Phase             = 4,
            Density           = PT_Density(; ρ0=3.2e3, α = α, T0 = 273),
            RadioactiveHeat   = ConstantRadioactiveHeat(0.022),
            ...
            CompositeRheology = CompositeRheology( 
                (
                    disl_wet_olivine, 
                    diff_wet_olivine,
                    DruckerPrager_regularised(; C = C, ϕ=ϕ_wet_olivine, η_vp=η_reg, Ψ=0.0) # non-regularized plasticity
                ) 
            ),
            # Elasticity        = el_upper_crust,
        ),
