...
function main3D(igg; ar = 8, ny = 16, nx = ny * 8, nz = ny * 8, figdir = "figs3D", do_vtk = false)
    ...
    compute_ρg!(ρg[3], phase_ratios, rheology, (T = thermal.T, P = stokes.P))
    ...
    let # let block to avoid polluting the global namespace
        ...
        scatter!(ax1, Array(thermal.T[2:(end - 1), 2:(end - 1), 2:(end - 1)][:]), Z ./ 1.0e3)
    ...
    dyrel = JustRelax3D.DYREL(
        backend_JR, stokes, rheology, phase_ratios, grid.di, dt; ϵ = 1.0e-6
    )
...
figdir = "3D_Benchmark_Duretz_etal_2014_DYREL"
