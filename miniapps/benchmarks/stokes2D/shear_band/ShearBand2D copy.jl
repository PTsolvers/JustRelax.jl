...
function main(igg; nx = 64, ny = 64, figdir = "model_figs")
    ...
    # Physical domain ------------------------------------
    lx = 1.0e0          # domain length in y
    ly = 0.7           # domain length in x
    ...
    τ_y = 1.74e-4      # yield stress. If do_DP=true, τ_y stand for the cohesion: c*cos(ϕ)
    ϕ = 30             # friction angle
    C = τ_y            # Cohesion
    η0 = 2.5e5         # viscosity
    G0 = 1.0           # elastic shear modulus
    Gi = 0.25          # elastic shear modulus perturbation
    εbg = 5e-10        # background strain-rate
    η_reg = 2.5e2      # regularisation "viscosity"
    dt = 1e4     # assumes Maxwell time of 4
    el_bg = ConstantElasticity(; G = G0, Kb = 2)
    el_inc = ConstantElasticity(; G = Gi, Kb = 2)
    ...
    for _ in 1:100
        ...
        heatmap!(ax2, xci..., Array(stokes.EII_pl), colormap = :inferno)
        heatmap!(ax3, xci..., Array(log10.(stokes.ε.II)), colormap = :inferno)
