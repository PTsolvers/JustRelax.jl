...
function main2D(igg; figdir = "Thermal_stresses", nx = 32, ny = 32, do_vtk = false)
    ...
    while it < 150
        ...
        # advect the free surface and flag the particles that crossed it
        semilagrangian_advection_markerchain!(chain, RungeKutta2(), @velocity(stokes), grid_vxi, grid.xvi, dt)
        update_phases_given_markerchain!(pPhases, chain, particles, grid.origin, grid.di.vertex, air_phase)
