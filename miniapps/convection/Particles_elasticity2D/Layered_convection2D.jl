...
function main2D(igg; ar=8, ny=16, nx=ny*8, figdir="figs2D", save_vtk =false)
    ...
    # Physical domain ------------------------------------
    thick_air    = 0e3              # thickness of sticky air layer
    ...
    τxx_p, τyy_p, τxy_p = init_particle_fields_cellarrays(particles, Val(3))
    vorticity_p,        = init_particle_fields_cellarrays(particles, Val(1))
    ...
    # Vertice vorticity array
    vorticity_vertex = @zeros(ni.+1...)
    vorticity_o_vertex = @zeros(ni.+1...)
    # Time loop
    t, it = 0.0, 0
    while it < 50 # run only for 5 Myrs
        ...
        particle2grid_centroid!(stokes.τ_o.xx,   τxx_p, xci, particles.coords)
        particle2grid_centroid!(stokes.τ_o.yy,   τyy_p, xci, particles.coords)
        particle2grid_centroid!(stokes.τ_o.xy_c, τxy_p, xci, particles.coords)
        # solve
        solve!(
            stokes,
            pt_stokes,
        ...
        for (src, src_o, dst_v, dst_v_o, dst_p) in zip(
            ...
            @parallel (1:nx+1) free_slip_x!(dst_v)
            @parallel (1:ny+1) free_slip_y!(dst_v)
            @parallel (1:nx+1) free_slip_x!(dst_v_o)
            @parallel (1:ny+1) free_slip_y!(dst_v_o)
            grid2particle_flip!(dst_p, xvi, dst_v, dst_v_o, particles.coords)
        end
        # interpolate vorticity to particles
        @parallel center2vertex!(vorticity_vertex, vorticity)
        @parallel center2vertex!(vorticity_o_vertex, vorticity_old)
        @parallel (1:nx+1) free_slip_x!(vorticity_vertex)
        @parallel (1:ny+1) free_slip_y!(vorticity_vertex)
        @parallel (1:nx+1) free_slip_x!(vorticity_o_vertex)
        @parallel (1:ny+1) free_slip_y!(vorticity_o_vertex)
        grid2particle_flip!(vorticity_p, xvi, vorticity_vertex, vorticity_o_vertex, particles.coords)
        # check if we need to inject particles
        inject = check_injection(particles)
        inject && inject_particles_phase!(particles, pPhases, (pT, τxx_p, τyy_p, τxy_p, vorticity_p), (T_buffer, τxx_vertex, τyy_vertex, stokes.τ.xy, vorticity_vertex), xvi)
        # rotate old stress
        @parallel (@idx ni...) JustRelax.Stokes2D.rotate_stress_particles_jaumann!(
            τxx_p, τyy_p, τxy_p, vorticity_p, particles.index, dt
        )
        # update phase ratios
        @parallel (@idx ni) phase_ratios_center(phase_ratios.center, pPhases)
