...
# backend trait
function solve_VariationalStokes_displacement!(stokes::JustRelax.StokesArrays, args...; kwargs)
    out = solve_VariationalStokes_displacement!(backend(stokes), stokes, args...; kwargs)
    return out
end

# entry point for extensions
function solve_VariationalStokes_displacement!(::CPUBackendTrait, stokes, args...; kwargs)
    return _solve_VS_displacement!(stokes, args...; kwargs...)
end

function _solve_VS_displacement!(
        stokes::JustRelax.StokesArrays,
        pt_stokes,
        di::NTuple{2, T},
        flow_bcs::AbstractFlowBoundaryConditions,
    ...
    while iter ≤ iterMax
        ...
        wtime0 += @elapsed begin
            ...
            # free_surface_bcs!(stokes, flow_bcs, η, rheology, phase_ratios, dt, di)
            flow_bcs!(stokes, flow_bcs)
            update_halo!(@displacement(stokes)...)
            # end
        end
        ...
        if iter % nout == 0 && iter > 1
            ...
            push!(err_evo1, err)
            push!(err_evo2, iter)
            err_it1 = maximum_mpi([norm_Rx[1], norm_Ry[1], norm_∇U[1]])
            rel_err = err / err_it1

            if igg.me == 0 #&& ((verbose && err > ϵ_rel) || iter == iterMax)
                @printf(
                    "Total steps = %d, abs_err = %1.3e , rel_err = %1.3e [norm_Rx=%1.3e, norm_Ry=%1.3e, norm_∇U=%1.3e] \n",
                    iter,
                    err,
                    rel_err,
                    norm_Rx[end],
