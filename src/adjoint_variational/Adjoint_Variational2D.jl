...
function _adjoint_solve_VSDot!(
    ...
    while (iter ≤ iterMax && err > ϵ)
        ...
        if SensType == "Vx"
            stokesAD.V.Vx[SensInd[1],SensInd[2]] .= -1.0#*norm # ∂J/∂Vx
        elseif SensType == "Vy"
            stokesAD.V.Vy[SensInd[1],SensInd[2]] .= -1.0#*norm # ∂J/∂Vy
        elseif SensType == "P"
            stokesAD.P[SensInd[1],SensInd[2]]    .= -1.0#*norm # ∂J/∂P
        end
        ...
        @parallel (@idx ni .+ 1) configcall=compute_Res!(
            ...
            #=
            compute_P!(
                stokesAD.PA,   # stokes.P in original use-case
                stokes.P0,
                stokes.R.RP,
                stokesAD.P,    # ΔV in original use-case
                stokes.Q,
                ητ,
                rheology,
                phase_ratios,
            ...
            # update λV
            @parallel (@idx ni) update_V!(   # (@idx ni .+ 1)
                stokesAD.VA.Vx,
                stokesAD.VA.Vy,
                stokesAD.V.Vx,
                stokesAD.V.Vy,
    ...
    Sens  = (G, Gv, fr, C)
    SensA = (stokesAD.G, Gvb, stokesAD.fr, stokesAD.C)
