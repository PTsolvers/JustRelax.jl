...
const np = haskey(ENV, "NP") ? parse(Int, ENV["NP"]) : 1
###
...
using ImplicitGlobalGrid, Printf, LinearAlgebra#, Plots, MAT
using MPI: MPI
...
macro av_xi_τr_dt(ix, iy, iz)
    return esc(:(max_lxyz / Vpdt / @av_xi_Re($ix, $iy, $iz) * Resc))
end
macro av_yi_τr_dt(ix, iy, iz)
    return esc(:(max_lxyz / Vpdt / @av_yi_Re($ix, $iy, $iz) * Resc))
end
macro av_zi_τr_dt(ix, iy, iz)
    return esc(:(max_lxyz / Vpdt / @av_zi_Re($ix, $iy, $iz) * Resc))
end
macro dt_ρ(ix, iy, iz)
    return esc(:(Vpdt * max_lxyz / @innH3($ix, $iy, $iz) / @Re($ix, $iy, $iz) * Resc))
end

@parallel_indices (ix, iy, iz) function compute_flux!(
    qHx, qHy, qHz, H, Vpdt, Resc, _dt, max_lxyz, max_lxyz2, _dx, _dy, _dz
)
    if (ix <= size(qHx, 1) && iy <= size(qHx, 2) && iz <= size(qHx, 3))
        qHx[ix, iy, iz] =
            (
                qHx[ix, iy, iz] * @av_xi_τr_dt(ix, iy, iz) -
                @av_xi_H3(ix, iy, iz) *
                _dx *
                (H[ix + 1, iy + 1, iz + 1] - H[ix, iy + 1, iz + 1])
            ) / (1.0 + @av_xi_τr_dt(ix, iy, iz))
    end
    if (ix <= size(qHy, 1) && iy <= size(qHy, 2) && iz <= size(qHy, 3))
        qHy[ix, iy, iz] =
            (
                qHy[ix, iy, iz] * @av_yi_τr_dt(ix, iy, iz) -
                @av_yi_H3(ix, iy, iz) *
                _dy *
                (H[ix + 1, iy + 1, iz + 1] - H[ix + 1, iy, iz + 1])
            ) / (1.0 + @av_yi_τr_dt(ix, iy, iz))
    end
    if (ix <= size(qHz, 1) && iy <= size(qHz, 2) && iz <= size(qHz, 3))
        qHz[ix, iy, iz] =
            (
                qHz[ix, iy, iz] * @av_zi_τr_dt(ix, iy, iz) -
                @av_zi_H3(ix, iy, iz) *
                _dz *
                (H[ix + 1, iy + 1, iz + 1] - H[ix + 1, iy + 1, iz])
            ) / (1.0 + @av_zi_τr_dt(ix, iy, iz))
    end
    return nothing
end
...
@views function diffusion_3D()
    ...
    # Numerics
    tol = 1e-8             # tolerance
    itMax = 200             # max number of iterations
    nout = 2000             # tol check
    ...
    if me == 0 && do_save
        ...
        open("./output/out_diff_3D_nonlin3_multixpu_perf_$(np).txt", "a") do io
            println(
                io,
