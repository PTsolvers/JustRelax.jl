...
function setup2D(
    ...
    add_box!(
        ...
        phase = LithosphericPhases(Layers = [chamber_depth + water_thickness], Phases = [1, 2]),
    ...
    if !flat
        add_volcano!(
            Phases, Temp, Grid;
            volcanic_phase = 1,
            center = (mean(Grid.x.val), -water_thickness),
            height = volcano_size[1],
            radius = volcano_size[2],
            crater = 0.5,
            base = -water_thickness,
            background = nothing,
            T = HalfspaceCoolingTemp(Age = 20)
        )
    end
    ...
    # add_ellipsoid!(
    #     Phases, Temp, Grid;
    #     cen = (mean(Grid.x.val), 0, -(chamber_depth + water_thickness - (chamber_radius / 2))),
    ...
    #     add_cylinder!(
    #         Phases, Temp, Grid;
    #         base = (mean(Grid.x.val), 0, -(chamber_depth + water_thickness - chamber_radius)),
...
function setup2D_Jacob(
        nx, nz;
        sticky_air     = 5.0,
        ice_thickness  = 2.5,
        dimensions     = (125.0e0, 125.0e0),
    )

    Lx = Ly = dimensions[1]
    x = range(0.0, Lx, nx)
    y = range(0.0, Ly, 2)
    z = range(-dimensions[2], sticky_air + ice_thickness, nz)
    Grid = CartData(xyz_grid(x, y, z))
    ...
    add_box!(
        Phases, Temp, Grid;
        xlim = (minimum(Grid.x.val), maximum(Grid.x.val) - Lx * 3 / 4),
        ylim = (minimum(Grid.y.val), maximum(Grid.y.val)),
        zlim = (minimum(Grid.z.val), ice_thickness),
        phase = LithosphericPhases(Layers = [20 + ice_thickness], Phases = [4]),
        T = HalfspaceCoolingTemp(Age = 20)
    )
    ...
    add_box!(
        ...
        phase = LithosphericPhases(Layers = [15, 30], Phases = [1, 2, 3]),
        T = HalfspaceCoolingTemp(Age = 20)
    )
...
# function extract_topo(grid, phases, air_phase)
#     phases_GMG = phases[:, 1, :]
#     topox      = grid.x.val[:,1,1]
#     z          = grid.z.val[:,1,:]
#     topoz      = zero(topox)

#     for i in 1:length(topox)
#         for j in 1:length(topoz)-1
#             if !(phases_GMG[i, j] == air_phase) && phases_GMG[i,j+1] == air_phase
#                 topoz[i] = z[i,j]
#                 break
#             end
#         end
#     end
#     topoz[end] = topoz[end - 1] # ensure last point is not NaN
#     # return Smoothing.binomial(topoz, 10)
#     return topoz .* 1e3
# end

function extract_topo(topox, z, phases_GMG, air_phase)
    topoz      = zeros(length(topox))

    for i in 1:length(topox)
        for j in 1:size(phases_GMG, 2)-1
            if !(phases_GMG[i, j] == air_phase) && phases_GMG[i,j+1] == air_phase
                topoz[i] = z[j]
