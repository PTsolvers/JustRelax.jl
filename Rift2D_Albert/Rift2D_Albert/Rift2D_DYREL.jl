...
function main(li, origin, T_GMG, phases_GMG, igg; nx=16, ny=16, figdir="figs2D", do_vtk =false)
    ...
    dtmax    = nondimensionalize(10e3 * yr, CharDim) # initial timestep in seconds
    # ----------------------------------------------------
    ...
    anomaly_center = nondimensionalize( (0e0, -20e3) .* m, CharDim)
    ...
    εbg = nondimensionalize(-1e-14/2 / s, CharDim)  # background strain rate 
    ...
    # Rheology
    stokes.ε.xx     .= 0 * εbg # to get a good initial guess
    ...
    dyrel = DYREL(backend_JR, stokes, rheology, phase_ratios, di, dt; ϵ = 1e-3)
...
figdir   = "output2_slow_softening/Rift2D_velocity_DYREL"
# n        = 160
# nx, ny   = n, n÷2
    # Lx = 500
    # Ly = 110
