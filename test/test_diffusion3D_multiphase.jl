...
@testset "Diffusion_3D_multiphase" begin
    @suppress begin
        ...
        if backend_JR == CPUBackend
            @test thermal.T[Int(ceil(nx/2)), Int(ceil(ny/2)), Int(ceil(nz/2))] ≈ 1825.7445437962856  rtol=1e-3
            @test thermal.Tc[Int(ceil(nx/2)), Int(ceil(ny/2)), Int(ceil(nz/2))] ≈ 1828.5560319263955 rtol=1e-3
        else
            @test true == true
        end
    end
